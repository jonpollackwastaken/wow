...
function General.LoadTooltip(tooltip, itemString)
	...
	if 1 then
		...
		local totalPrice, totalSaleNum = TSM.Accounting.Transactions.GetSaleStats(itemString)
		...
		numHave = numHave + (alts or 0) + (altsAH or 0)


		totalSaleNum = totalSaleNum or 0
		totalFailed = totalFailed or 0
		smartBuyPrice = smartBuyPrice or 0
		smartBuyNum = smartBuyNum or 0
		totalPrice = totalPrice or 0

		
		local avgBuyPrice = (smartBuyPrice and smartBuyPrice > 0 and smartBuyNum and smartBuyNum > 0) and Math.Round(smartBuyPrice / smartBuyNum) or 0
		local avgSalePrice = totalPrice and Math.Round(totalPrice / totalSaleNum) or 0
		...
		local lifetime = roi / 100 * avgBuyPrice * totalSaleNum
		...
		if not string.match(roi, "nan") then 
			tooltip:AddLine(L["Stats"], format("|cffffffff"..roi.."%%|r ROI |cffffffff"..saleRate.."%%|r SR"))
			tooltip:AddLine(L[" "], format("Lifetime %s", tooltip:FormatMoney(lifetime)))
		end
